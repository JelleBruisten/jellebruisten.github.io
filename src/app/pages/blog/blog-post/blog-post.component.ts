...
@Component({
  ...
  template: `
    <div class="max-w-4xl mx-auto px-4 sm:px-6 py-16">
      ...
      <a
        routerLink="/blog"
        class="inline-flex items-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-brand-dark dark:hover:text-brand-light transition-colors mb-8"
      >
      ...
      @if (p) {
        ...
        >
            ...
            <div class="flex flex-wrap gap-2 mb-6">
              @for (tag of p.tags; track tag) {
                <a
                  [routerLink]="['/blog']"
                  [queryParams]="{ tag }"
                  class="text-xs font-medium px-2.5 py-1 rounded-full transition-colors bg-brand/8 dark:bg-brand-light/8 text-brand-dark dark:text-brand-light hover:bg-brand/20 dark:hover:bg-brand-light/20"
                >
                  {{ tag }}
                </a>
              }
          ...
          <!-- Title -->
          <h1
            class="text-3xl sm:text-4xl font-bold tracking-tight text-slate-900 dark:text-white leading-tight mb-4"
          >
            {{ p.title }}
          </h1>
          ...
          <!-- Meta -->
          <div
            class="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400 mb-10 pb-8 border-b border-slate-200/60 dark:border-slate-700/60"
          >
          ...
          @if (safeContent()) {
            <div class="prose text-slate-700 dark:text-slate-300" [innerHTML]="safeContent()"></div>
          } @else {
            <div
              class="flex items-center gap-3 py-12 justify-center text-slate-400 dark:text-slate-500"
            >
        ...
        <div class="mt-8">
          <a
            routerLink="/blog"
            class="inline-flex items-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-brand-dark dark:hover:text-brand-light transition-colors"
          >
            <app-arrow-left-icon class="text-[16px]" />
            All posts
          </a>
        ...
        <!-- Not found -->
        <div
          class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md rounded-2xl border border-slate-200/60 dark:border-slate-700/60 p-12 text-center"
        >
          ...
          <a
            routerLink="/blog"
            class="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-medium bg-brand-dark text-white hover:brightness-90 transition-all hover:scale-105 active:scale-95"
          >
            Back to blog
          </a>
        </div>
...
export class BlogPostComponent {
  ...
  private contentHtml = toSignal(
    toObservable(this.slug).pipe(
      switchMap((slug) => (slug ? this.blog.getPostContent(slug) : of(null))),
    ),
    { initialValue: null },
  );
